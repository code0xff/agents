...
export function PaymentGraph({ payments, counts }: { payments: Payment[]; counts: Record<string, number> }) {
  ...
  useEffect(() => {
    ...
    const linkE = link.enter().append('line').attr('stroke', '#cfcfd6').attr('opacity', 0)
    ...
    nodeAll.select('circle').transition().duration(500).attr('r', r)
      .attr('fill', (d) => d.kind === 'facilitator' ? '#fafafa' : d.kind === 'payTo' ? '#9d9da6' : '#2c2c31')
      .attr('stroke', (d) => d.kind === 'payer' ? '#6f6f78' : 'none').attr('stroke-width', 1)
    nodeAll.select('text').text((d) => d.kind === 'facilitator' ? d.label : d.kind === 'payTo' && d.weight >= 3 ? d.label : '')
      .attr('fill', (d) => d.kind === 'facilitator' ? '#fafafa' : '#9d9da6')
  ...
  useEffect(() => {
    ...
    fresh.slice(0, 12).forEach((pay, i) => {
      const hop = (a: string, b: string, delay: number) => {
        ...
        if (!A || !B) return
        const c = g.append('circle').attr('r', 3).attr('fill', '#fafafa').attr('cx', A.x!).attr('cy', A.y!).attr('opacity', 0)
        ...
        g.append('circle').attr('r', 4).attr('fill', 'none').attr('stroke', '#fafafa').attr('cx', B.x!).attr('cy', B.y!).attr('opacity', 0)
